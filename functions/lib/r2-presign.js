...
/**
 * Create a presigned GET URL for R2 so third-party processors can fetch private objects.
 */
export async function createR2PresignedGet(env, { key, expiresIn = 3600 }) {
  const accessKeyId = env.R2_ACCESS_KEY_ID;
  const secretAccessKey = env.R2_SECRET_ACCESS_KEY;
  const accountId = env.R2_ACCOUNT_ID;
  const bucket = env.R2_BUCKET || env.R2_BUCKET_NAME || 'content-station-media';
  ...
  if (!accessKeyId || !secretAccessKey || !accountId) {
    return {
      ...
      error: 'r2_s3_unconfigured',
      message:
        'Presigned downloads need R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY, and R2_ACCOUNT_ID in Pages secrets.',
    };
  }
  if (!key || typeof key !== 'string') {
    return { ok: false, error: 'invalid_key', message: 'Missing object key.' };
  }

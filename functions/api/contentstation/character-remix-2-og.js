...
export async function onRequest(context) {
  ...
  if (action === 'from-tiktok') {
    ...
    // Always strip from every URL list + blocklist so Autogenerate never picks them again.
    if (isTikTokPhotoUrl(tiktokUrl)) {
      ...
      try {
        removed = await removeUrlsFromAllLists(env, [tiktokUrl]);
      } catch {
        removed = null;
      }
      try {
      ...
      return json(
        {
          ...
          message:
            'TikTok photo/slideshow posts cannot be remixed (no video keyframes). Removed from all lists + blocklisted. Autogenerate will try another leftover.',
          removedFromList: Boolean(removed?.ok && (removed.removed || 0) > 0),
          removedCount: Number(removed?.removed || 0),
          removedByList: removed?.byList || [],
          blocklisted,
          listId: fromListId,
        },
        422,
